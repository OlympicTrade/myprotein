...
class OrdersService extends AbstractService
{
    ...
    public function orderStep2($data)
    {
        ...
        if($data['attrs-delivery'] == 'pickup') {
            $data['attrs-date'] = $this->getDeliveryService()->getDeliveryDates([
                ...
                //'to_index' => $order->getPlugin('region')->get('index'),
    ...
    public function orderMsg($order)
    {
        ...
        if($attrs->get('delivery') == 'pickup') {
            ...
            $sms->send(
                ...
                'Заказ ' . $order->getPublicId() . ' (' . $order->getPrice() . ' rub) Самовывоз ' . $point->get('address')
            ...
            $sms->send(
                $order->getPlugin('phone')->get('phone'),
                'Заказ ' . $order->getPublicId() . ' (' . $order->getPrice() . ' rub) Доставка ' . $attrs->get('date') . ' с ' . $attrs->get('time_from') . ' до ' . $attrs->get('time_to')
            );
        } else {
            ...
            $sms->send(
                $order->getPlugin('phone')->get('phone'),
                'Заказ ' . $order->getPublicId() . ' (' . $order->getPrice() . ' rub). Требуеться оплата для отправки по адресу: ' . $attrs->get('index') . ' ' . $attrs->get('address')
            );
        }
    ...
    public function updateOrderPrice(Order $order)
    {
        ...
        foreach ($order->getPlugin('cart') as $cart) {
            ...
            $weight += $cart->getPlugin('product')->get('weight');
        }
    ...
    public function changeProductCount($productId, $tasteId, $sizeId, $count) {
        ...
        $this->getSyncService()->addProductToSync($where + ['diff' => $count]);
